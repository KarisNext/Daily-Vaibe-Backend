...
class GeoService {
    ...
    try {
      ...
      const categoryResult = await pool.query(`
        SELECT 
          ...
          COUNT(DISTINCT CASE WHEN county IS NOT NULL AND county != 'Unknown' THEN county END) as total_counties,
          COUNT(DISTINCT CASE WHEN town IS NOT NULL AND town != 'Unknown' THEN town END) as total_towns,
          COALESCE(SUM(visit_count), 0) as total_visits,
          COUNT(CASE WHEN last_seen > NOW() - INTERVAL '15 minutes' THEN 1 END) as active_now
        FROM session_geo 
        GROUP BY category
      ...
      console.error('GeoService.getGeoStats error:', error);
      return { 
        success: false, 
        error: error.message,
        byCategory: [],
  ...
  static async getAllDevices(filters = {}) {
    try {
      ...
      console.error('GeoService.getAllDevices error:', error);
  ...
  static async getActiveDevices() {
    try {
      ...
      console.error('GeoService.getActiveDevices error:', error);
  ...
  static async getCountyDetails(county) {
    try {
      ...
      console.error('GeoService.getCountyDetails error:', error);
    ...
    try {
      ...
      const result = await pool.query(`
        SELECT 
          ...
          COUNT(DISTINCT CASE WHEN county IS NOT NULL AND county != 'Unknown' THEN county END) as daily_counties,
          COALESCE(SUM(visit_count), 0) as daily_visits
        FROM session_geo 
      ...
      console.error('GeoService.getGeoTrends error:', error);
  ...
  static async cleanupOldSessions(daysOld = 30) {
    try {
      ...
      console.error('GeoService.cleanupOldSessions error:', error);
  ...
  static async updateDeviceInfo(sessionId, updates) {
    try {
      ...
      console.error('GeoService.updateDeviceInfo error:', error);
      return { success: false, error: error.message };
    }
  }
}
